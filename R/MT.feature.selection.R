...
MT.feature.selection <- function(distance_list, MT_variants_detected_matrix, dis_cutoff = 0.001, sample_type = 1 , size = 5 ){
  ...
  if(length(tmp.selected.MT.variants)==1) {

    final.MT.variant = tmp.selected.MT.variants
    #return(final.MT.variant)

  } else if(length(tmp.selected.MT.variants)==0){
    final.MT.variant = "No informative MT mutations under this cutoff!"
    #return(final.MT.variant)
